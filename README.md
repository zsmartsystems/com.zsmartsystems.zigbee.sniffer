...
This project uses the ```com.zsmartsystems.zigbee.dongle.ember``` driver for Silicon Labs Ember (EM35x and EFR32) based dongles to provide a Zigbee sniffer interface for Wireshark, and optionally write the data to a Silabs compatible event log. The software will connect to an Ember dongle using a serial interface, and send UDP packets on port 17754 which can be received and displayed by Wireshark.
